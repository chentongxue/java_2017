...
import com.game.draco.app.skill.func.GoddessSkillLearnFunc;
...
public class UserSkillAppImpl implements UserSkillApp {
	...
	public UserSkillAppImpl(){
		...
		this.register(new GoddessSkillLearnFunc());
	...
	public SkillApplyResult useSkill(AbstractRole role, short skillId,int clientDelay,
		...
		context.setSetEffectTime(false);
		return skill.use(context);
	}
	...
	private void notifyActiveSkill(RoleInstance role){
		try {
			...
			if(null == levelup || !levelup.isBaseMaxChanged()){
				//当前级别相对于前一级的消耗没有发生变化
				return ;
			}
			Map<Short, RoleSkillStat> skillMap = role.getSkillMap();
	...
	public void login(AbstractRole role) {
		try{
			...
			if(null == onBattleHero){
				//主角普通攻击
				short cmSkillId = GameContext.getSkillConfig().getRoleCommonSkillId();
				RoleSkillStat cmStat = new RoleSkillStat();
				cmStat.setRoleId(roleId);
				cmStat.setSkillId(cmSkillId);
				cmStat.setSkillLevel(1);
				skillMap.put(cmSkillId, cmStat);
			}else{
			...
			if(roleHorse != null){
				skillMap.putAll(roleHorse.getSkillMap());
			}
