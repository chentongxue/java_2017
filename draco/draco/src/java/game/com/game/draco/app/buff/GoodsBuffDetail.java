...
public @Data class GoodsBuffDetail extends BuffDetail{
	
	private int addHp;//立即回复HP值
	private int addHpRate;//立即回复HP百分比(相对最大)
	private int addHpBaseRate;//立即回复HP百分比(相对基本)
	private int addMp;//立即回复MP值
	private int addMpRate;//立即回复MP百分比(相对最大mp)
	private int addMpBaseRate ;//立即回复MP百分比(相对基本mp)
	...
	private int everyAddHpBaseRate;//BUFF每跳回复HP百分比(相对基本)
	private int everyAddMp;//BUFF每跳回复MP值
	private int everyAddMpRate;//BUFF每跳回复MP百分比
	private int everyAddMpBaseRate;//BUFF每跳回复MP百分比(相对基本)
	...
	public @Data class AddAttriItem {
		...
		public AttriItem toAttriItem(AbstractRole role){
			...
			if(this.baseRate != 0){
				int base = this.getBase(attriType, role);
				total += (int)(this.baseRate/TEN_THOUSAND*base) ;
			}
			if(0 == total){
				return null ;
			}
		...
		private int getBase(AttributeType type,AbstractRole role){
			if(role.getRoleType() != RoleType.PLAYER){
				return 0 ;
			}
			RoleInstance player = (RoleInstance)role ;
			RoleLevelup levelUp = GameContext.getAttriApp().getLevelup(role.getLevel());
			if(null == levelUp){
				return 0 ;
			}
			if(AttributeType.curHP == type){
				return levelUp.getBaseMaxHp();
			}
			if(AttributeType.curMP == type){
				return levelUp.getBaseMaxMp();
			}
			return 0 ;
		}
	...
	public void init(){
	 ...
	 private AddAttriItem create(AttributeType attriType, int value,
			int maxRate, int baseRate) {
		 if(null == attriType ){
			 return null ;
		 }
		 if(0 == value && 0 == maxRate && 0 == baseRate){
			 return null ;
		 }
		return new AddAttriItem(attriType,value,maxRate,baseRate) ;
	}
	...
	private List<AddAttriItem> getBeginAddAttriItems(){
		...
		AddAttriItem item = this.create(AttributeType.curHP, addHp, addHpRate, addHpBaseRate);
		if(null != item){
			values.add(item);
		}
		item = this.create(AttributeType.curMP, addMp, addMpRate, addMpBaseRate);
		if(null != item){
			values.add(item);
		}
		/*item = this.create(AttributeType.angerValue, addAnger, 0, 0);
		if(null != item){
			values.add(item);
		}*/
		return values;
	...
	private List<AddAttriItem> getProcessAddAttriItems(){
		List<AddAttriItem> values = new ArrayList<AddAttriItem>();
		AddAttriItem item = this.create(AttributeType.curHP,everyAddHp, everyAddHpRate, everyAddHpBaseRate);
		if(null != item){
			values.add(item);
		}
		item = this.create(AttributeType.curMP, everyAddMp, everyAddMpRate, everyAddMpBaseRate);
		if(null != item){
			values.add(item);
		}
		/*item = this.create(AttributeType.angerValue, everyAddAnger, 0, 0);
		if(null != item){
			values.add(item);
		}*/
		return values;
