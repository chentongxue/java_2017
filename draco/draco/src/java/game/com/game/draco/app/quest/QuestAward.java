...
import lombok.Data;
import sacred.alliance.magic.app.faction.integral.QuestAwardChannel;
...
//			}
			...
			if(AttributeType.silverMoney == attrType){
				silverMoney += value;
			}
		}
		if(notify){
		...
		//打日志
		if(exp > 0 || silverMoney > 0){
			GameContext.getStatLogApp().roleQuest(role, quest, exp, silverMoney);
		}
	}
