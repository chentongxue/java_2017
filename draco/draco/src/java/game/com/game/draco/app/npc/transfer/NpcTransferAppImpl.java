...
public class NpcTransferAppImpl implements NpcTransferApp{
	...
	@Override
	public String transferRole(RoleInstance role, String param){
		try {
			...
			if(!suit){
				return GameContext.getI18n().getText(TextId.TRANSFER_NOTSUIT_CONDITION); 
			}
			//消耗不足
			...
			if(titleId != 0 && 
				...
				return GameContext.getI18n().messageFormat(TextId.TRANSFER_ASK_TITLE,name);
			}
			//是否有对应的物品
			...
			if(goodsId > 0){
				...
				if(ownNum < goodsNum){
					...
					return GameContext.getI18n().messageFormat(TextId.TRANSFER_GOODS_NOT_ENOUGH, goodsNum, goodsName);
				}
			}
