...
public class UserQuestAppImpl implements UserQuestApp {
	...
	@Override
	public void login(RoleInstance role) throws ServiceException {
		try {
			...
			throw new ServiceException("task init load failed", e);
		}
	}
    ...
    private void completeQuestAction(RoleInstance role,Quest quest) throws ServiceException {
		...
		//完成任务时给角色增加Buff
		if(null != GameContext.getBuffApplication().getBuff(quest.getCompleteBuff())){
