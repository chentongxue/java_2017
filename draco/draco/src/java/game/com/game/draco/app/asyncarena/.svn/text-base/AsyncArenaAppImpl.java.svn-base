...
public class AsyncArenaAppImpl implements AsyncArenaApp{
	...
	//异步竞技场赛场描述
	@Getter @Setter private List<AsyncMap> mapList = null;
	...
	private void loadAsyncMapConfig(){
		try{
			...
			String sourceFile = GameContext.getPathConfig().getXlsPath() + fileName;
			//异步竞技场筛选排序规则数据
			mapList = XlsPojoUtil.sheetToList(sourceFile, sheetName, AsyncMap.class);
			if(mapList == null || mapList.isEmpty()){
				Log4jManager.CHECK.error("not config the mapList,file=" + sourceFile + " sheet=" + sheetName);
				Log4jManager.checkFail();
			}else{
				for(AsyncMap m : mapList){
					sacred.alliance.magic.app.map.Map map = GameContext.getMapApp().getMap(m.getMapId());
	...
	@Override
	public List<AsyncMap> getAsyncMapList() {
		return mapList;
	}
