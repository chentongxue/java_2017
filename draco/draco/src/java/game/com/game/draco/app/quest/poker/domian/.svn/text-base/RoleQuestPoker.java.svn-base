...
public @Data class RoleQuestPoker {
	...
	public void completeQuest(RoleInstance role){
		...
		if(this.isFinishedAll()){
			//三张牌的奖励
			GameContext.getQuestPokerApp().sendThreePokerReward(role, this.getPokerValue(this.poker1), this.getPokerValue(this.poker2), this.getPokerValue(this.poker3));
			this.count ++;
			int maxCount = GameContext.getQuestPokerApp().getMaxCount();
			//达到最大次数后不清空牌型记录
