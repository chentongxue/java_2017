...
public class MedalAppImpl implements MedalApp{
	...
	public void login(RoleInstance role){
		try{
			...
			//强化
			int minStrengthenLevel = role.getEquipBackpack().minEffectStrengthenLevel();
			MedalConfig strengthenCfg = this.getStrengthenConfig(minStrengthenLevel);
	...
	private MedalConfig getStrengthenConfig(int minStrengthenLevel){
		if(0 == minStrengthenLevel){
			return null ;
		}
		...
		for(int i = size-1 ;i >= 0 ;i--){
			...
			if(minStrengthenLevel >= config.getLevel()){
				return config ;
			}
		}
		return null ;
	...
	private MedalConfig getMosaicConfig(RoleInstance role){
		...
		for(int i = size-1 ;i >= 0 ;i--){
			MedalConfig config  = list.get(i);
			int roleNum = role.getEquipBackpack().totalEffectMosaicLevel(config.getLevel());
	...
	private MedalConfig getRecastingConfig(RoleInstance role){
		...
		for(int i = size-1 ;i >= 0 ;i--){
			MedalConfig config  = list.get(i);
			int roleNum = role.getEquipBackpack().totalEffectRecastingQualityNum(config.getLevel());
			if(roleNum >= config.getNum()){
				return config ;
			}
		}
	...
	private void doUpdateMedal(RoleInstance role, MedalType medalType){
		try{
			...
			if(MedalType.QiangHua == medalType){//强化等级
				int minStrengthenLevel = role.getEquipBackpack().minEffectStrengthenLevel();
				nowConfig = this.getStrengthenConfig(minStrengthenLevel);
			}else if(MedalType.XiangQian == medalType){//镶嵌
				nowConfig = this.getMosaicConfig(role);
			}else if(MedalType.XiLian == medalType){//洗练
				//int recastingStar = role.getEquipBackpack().totalEffectRecastingQualityNum();
	...
	public C0521_MedalListRespMessage getC0521_MedalListRespMessage(RoleInstance role){
		...
		for(Map.Entry<Integer, Integer> entry : rd.getRoleMedalMap().entrySet()){
			...
			MedalType medalType = MedalType.get(entry.getKey());
			int index = entry.getValue();
			MedalConfig config = GameContext.getMedalApp().getMedalConfig(medalType, index);
			MedalIconItem item = new MedalIconItem();
			short icon; 
			if(null == config){
				icon = GameContext.getMedalApp().getDefaultIcon(medalType);
			}else{
		...
		message.setMedalList(medalList);
		return message;
	}
	...
	public void updateMedal(RoleInstance role, MedalType medalType, RoleGoods roleGoods){
		try {
			if(null != roleGoods){
				...
				if(null == ge || !EquipslotType.get(ge.getEquipslotType()).isEffect()){
					return;
				}
			}
	...
	public AttriBuffer getAttriBuffer(AbstractRole player) {
		...
		try{
			...
			//强化等级
			int minStrengthenLevel = role.getEquipBackpack().minEffectStrengthenLevel();
			MedalConfig strengConfig = this.getStrengthenConfig(minStrengthenLevel);
	...
	public void logout(RoleInstance role) {
		try {
			this.roleMedalMap.remove(role.getRoleId());
		} catch (Exception e) {
