...
public class MailAppImpl implements MailApp{
	...
	private Result pickMailAccessory(RoleInstance role, Mail mail, boolean isPayGold)throws ServiceException{
		try{
			...
			int contribute = mail.getContribute();
			int zp = mail.getZp();
			...
			//提取邮件日志
			GameContext.getStatLogApp().mailSendInfoPickLog(role, mail, maList, exp, gold, bindGold, silverMoney, contribute, zp, 0);
			return result.setResult(Status.SUCCESS.getInnerCode());
		}catch(Exception e){
	...
	private Result getMailAttri(Mail mail, RoleInstance role){
		...
		try{
			...
			if(bindGold > 0){
				GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.bindingGoldMoney, OperatorType.Add, bindGold, ocType);
				notify = true ;
			}
			if(silverMoney > 0){
				GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.silverMoney, OperatorType.Add, silverMoney, ocType);
				notify = true ;
			}
//			if(contribute > 0){
//				GameContext.getFactionFuncApp().changeContributeNum(role, OperatorType.Add, contribute);
//				notify = true ;
//			}
			if(zp > 0){
				GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.potential, OperatorType.Add, zp, ocType);
				notify = true ;
			}
			//提取过的金钱和经验，重置为0
			mail.setBindGold(0);
	...
	public void sendMail(String toRoleId, String title, String content, String sendRole,
		...
		this.generalSendMail(toRoleId, title, content, sendRole, sendSource, goodsList, attriBean, false);
	}
	
	@Override
	public void sendMail(String toRoleId, String title, String content, 
			String sendRole, int sendSource, int goldMoney, int bindGoldMoney, 
			int silverMoney, List<GoodsOperateBean> goodsList) {
		MailAttriBean bean = null ;
		if(goldMoney != 0 || bindGoldMoney !=0 || silverMoney != 0){
			bean = new MailAttriBean();
			bean.setGold(goldMoney);
			bean.setBindGold(bindGoldMoney);
	 ...
	 * 同步、异步发邮件
	 */
	private void generalSendMail(String toRoleId, String title, String content, String sendRole,
			int sendSource, List<GoodsOperateBean> goodsList, MailAttriBean attriBean, boolean isAsync){
		try {
			...
			if(null != attriBean){
				...
				mail.setContribute(attriBean.getContribute());
				mail.setZp(attriBean.getZp());
	...
	public void sendMailAsync(Mail mail) {
		try {
			...
			GameContext.getMessageCenter().send("", mail.getRoleId(), message);
		} catch (Exception e) {
	...
	public void sendMailAsync(String toRoleId, String title, String content,
			String sendRole, int sendSource, int goldMoney, int exp, int bindGoldMoney,
			int silverMoney, List<GoodsOperateBean> goodsList) {
		MailAttriBean bean = null ;
		if(goldMoney != 0 || bindGoldMoney !=0 || silverMoney != 0 || exp != 0){
			bean = new MailAttriBean();
			bean.setGold(goldMoney);
			bean.setBindGold(bindGoldMoney);
			bean.setSilverMoney(silverMoney);
	...
	public void sendMailAsync(String toRoleId, String title, String content,
			String sendRole, int sendSource, List<GoodsOperateBean> goodsList) {
		this.sendMailAsync(toRoleId, title, content, sendRole, sendSource, goodsList, null);
	}

	@Override
	public void sendMailAsync(String toRoleId, String title, String content, String sendRole, 
			int sendSource, List<GoodsOperateBean> goodsList, MailAttriBean attriBean) {
		this.generalSendMail(toRoleId, title, content, sendRole, sendSource, goodsList, attriBean, true);
	}
