...
public class RoleAsyncArenaAppImpl implements RoleAsyncArenaApp {
	...
	public final static int NUM = 16;
		...
		try {
			...
			if(groupMap != null && !groupMap.isEmpty()){
				...
				for(Entry<Byte,AsyncGroup> group : groupMap.entrySet()){
					...
					String gtr = "";String ltr = "";
					if(gt != 0){
						gtr = String.valueOf(gt);
					}
					if(lt != 0){
						ltr = String.valueOf(lt);
					}
					Map<String,String> data = GameContext.getAsyncPvpApp().getRoleBattleScores(role.getRoleId(), gtr,ltr,100);
	...
	public NpcInstance createAsyncArenaNpc(RoleInstance role, int targetRoleId,
		...
		try {
			...
			AsyncPvpRoleAttr attr = GameContext.getAsyncPvpApp()
					.getAsyncPvpRoleAttr(String.valueOf(targetRoleId));
			List<AsyncMap> mapList = GameContext.getAsyncArenaApp()
					.getAsyncMapList();
			npcInstance = NpcInstanceFactroy.createAsyncPvpNpcInstance(attr,
					mapList.get(0).getMapId(), mapList.get(0).getTargetMapX(),
					mapList.get(0).getTargetMapY());
			...
			for (RoleInstance ri : mapInstance.getRoleList()) {
				...
				message.setItem(Converter.getAsyncPvpRoleBodyItem(npcInstance
						.getRoleId(), attr, (short) mapList.get(0)
						.getTargetMapX(), (short) mapList.get(0)
						.getTargetMapY()));
				...
				if(sortList != null && !sortList.isEmpty()){
					...
					for(AsyncSort sort : sortList){
						...
						if(ruleNum == sortList.size()){
							refNum = (NUM - asyncBattleInfoMap.size());
						}
	...
	private long getRoleAsyncArenaRanking(){
		return GameContext.getAsyncPvpApp().getTotalRanking();
	}
...
//		result.setInfo(GameContext.getI18n().messageFormat(TextId.ASYNC_ARENA_ROLE_REF_SUCCESS));
	...
	public void saveOrUpdRoleAsyncArena(RoleInstance role,AsyncArenaRole asyncArenaRole){
		try{
			...
			GameContext.getAsyncPvpApp().saveRoleAsyncArena(String.valueOf(asyncArenaRole.getRoleId()), asyncArenaRole.getHistoryHonor()+asyncArenaRole.getNowHonor());
		}catch(Exception e){
	...
	public void initRoleAsyncArena(RoleInstance role){
		try{
			if(!roleAsyncArenaMap.containsKey(role.getIntRoleId())){
				if(GameContext.getMenuApp().isOpenFun(role, MenuIdType.AsyncArena)){
	...
	private void initRoleAsyncArenaMap(RoleInstance role){
		try{
			// 初始化角色坐骑数据
			AsyncArenaRole asyncArenaRole = GameContext.getBaseDAO().selectEntity(
					AsyncArenaRole.class, AsyncArenaRole.ROLE_ID,role.getIntRoleId());
			if(asyncArenaRole != null){
			...
			if(asyncArenaRole != null){
				//存排行榜
				GameContext.getAsyncPvpApp().saveRoleAsyncArena(role.getRoleId(), asyncArenaRole.getHistoryHonor()+asyncArenaRole.getNowHonor());
			}
		}catch(Exception e){
	...
	public void challengeOver(RoleInstance role, AsyncPvpBattleInfo battleInfo, ChallengeResultType type) {
		...
		try {
			...
			if(type == ChallengeResultType.Win) {
				...
				GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.silverMoney, OperatorType.Add, aysncReward.getGoldMoney(), OutputConsumeType.async_arena_role_add_money);
	...
	public Map<Integer,AsyncBattleInfo> parseData(byte [] targetData) {
		...
		try {
			...
			for(int i=0;i<size;i++){
			 	AsyncBattleInfo battleInfo = new AsyncBattleInfo();
			 	battleInfo.setTroleId(in.readInt());
			 	battleInfo.setIsSu(in.readByte());
			 	battleInfo.setState(in.readByte());
			 	battleInfo.setCTime(in.readLong());
			 	asyncBattleInfoMap.put(battleInfo.getTroleId(), battleInfo);
			}
			in.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public byte [] buildData(Map<Integer,AsyncBattleInfo> asyncBattleInfoMap) {
		try {
			...
			out.writeByte(asyncBattleInfoMap.size());
			for(Entry<Integer,AsyncBattleInfo> battleInfo : asyncBattleInfoMap.entrySet()){
				out.writeInt(battleInfo.getKey());
				out.writeByte(battleInfo.getValue().getIsSu());
				out.writeByte(battleInfo.getValue().getState());
				out.writeLong(battleInfo.getValue().getCTime());
			}
			out.flush();
			out.close();
	...
	public void challenge(RoleInstance role, int targetRoleId) {
		try {
			...
			//切换地图 
			List<AsyncMap> map = GameContext.getAsyncArenaApp().getAsyncMapList();
			Point targetPoint = new Point(map.get(0).getMapId(), map.get(0).getMapX(), map.get(0).getMapY());
	...
	@Override
	public AsyncArenaRoleBuyChallengeItem buyChallengeNum(RoleInstance role) {
		...
		int silverMoney = asyncBuy.getPrice();
		if(silverMoney > 0) {
			if(role.getSilverMoney() < silverMoney) {
				//提示游戏币不足
				result.setNum(asyncArenaRole.getChallengeNum());
				result.setExpenditure(oldAsyncBuy.getPrice());
				result.setMsg(GameContext.getI18n().messageFormat(TextId.ASYNC_ARENA_BUY_CHALLENGE_COUNT_FAIL, silverMoney));
				return result ;
			}
		}
		...
		//扣除消耗
		GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.silverMoney, 
				OperatorType.Decrease, silverMoney, OutputConsumeType.async_arena_role_consume_money);
		role.getBehavior().notifyAttribute();
		...
		result.setMsg(GameContext.getI18n().messageFormat(TextId.ASYNC_ARENA_BUY_CHALLENGE_COUNT_SUCCESS, silverMoney));
	...
	private boolean sendRankAward(RoleInstance role, int ranking) {
		...
		if(rankReward != null){
			...
			GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.silverMoney, OperatorType.Add, rankReward.getGoldMoney(), OutputConsumeType.async_arena_role_add_money);
			GameContext.getUserAttributeApp().changeAttribute(role, AttributeType.exp,OperatorType.Add , rankReward.getExp(), OutputConsumeType.async_arena_role_add_exp);
			GameContext.getUserAttributeApp().changeAttribute(role, AttributeType.potential,OperatorType.Add , rankReward.getZp(), OutputConsumeType.async_arena_role_add_potential);
			GameContext.getUserAttributeApp().changeAttribute(role, AttributeType.honor,OperatorType.Add , rankReward.getHonor(), OutputConsumeType.async_arena_role_add_honor);
			role.getBehavior().notifyAttribute();
			return true;
		}
	...
	public List<AsyncArenaTargetItem> getAsyncArenaTargetItemList(RoleInstance role){
		...
		for(Entry<Integer,AsyncBattleInfo> battleInfo : asyncBattleInfoMap.entrySet()){
			...
			targetItem.setResId(attr.getHeroResId());
			targetItem.setState(battleInfo.getValue().getState());
			targetItem.setIsSu(battleInfo.getValue().getIsSu());
			HorseBase horse = GameContext.getHorseApp().getHorseBaseById(attr.getHorseId());
			if(horse != null){
				targetItem.setHorseName(horse.getName());
				targetItem.setHorseLevel(attr.getHorseLevel());
				targetItem.setHorseResId(horse.getIconId());
				targetItem.setHorseQuality(attr.getHorseQuality());
			}
			GoodsBase hero = GameContext.getGoodsApp().getGoodsBase(attr.getHeroId());
			if(hero != null){
				targetItem.setHeroName(hero.getName());
				targetItem.setHeroLevel(attr.getHeroLevel());
				targetItem.setHeroResId((short)hero.getResId());
				targetItem.setHeroQuality(attr.getHeroQuality());
			}
			GoodsGoddess goodsGoddess = GameContext.getGoddessApp().getGoddessName(attr.getGoddessId());
			if(goodsGoddess != null){
				targetItem.setGoddessName(goodsGoddess.getName());
				targetItem.setGoddessLevel(attr.getGoddessLevel());
				targetItem.setGoddessResId((short)goodsGoddess.getResId());
				targetItem.setGoddessQuality(attr.getGoddessQuality());
			}
