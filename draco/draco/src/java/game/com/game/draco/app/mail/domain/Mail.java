...
public @Data class Mail {
	...
	/***********非数据库字段*****/
	public static final int MaxAccessoryNum = 5;//邮件最大附件数量
	...
	public boolean isExistAttri(){
		return gold != 0 || bindGold != 0 || silverMoney != 0 || exp != 0 || zp != 0 || contribute != 0 ;
	}
	...
	private void addAccessoryInfo(){
		try{
			...
			if(this.bindGold > 0){
				info.append(AttributeType.bindingGoldMoney.getName());
				info.append("*");
				info.append(bindGold);
				info.append("\n");
			}
			if(this.silverMoney > 0){
				info.append(AttributeType.silverMoney.getName());
				info.append("*");
...
//				info.append(AttributeType.contribute.getName());
//				info.append("*");
//				info.append(contribute);
//				info.append("\n");
//			}
			if(this.zp > 0){
