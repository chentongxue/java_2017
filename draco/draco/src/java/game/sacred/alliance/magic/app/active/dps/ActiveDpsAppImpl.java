...
public class ActiveDpsAppImpl implements ActiveDpsApp {
	...
	private void initActive(){
		...
		for(Active active : list){
			if(ActiveType.Dps.getType() != active.getType()){
				continue ;
			}
	...
	@Override
	public void sendReward(short activeId, RoleInstance role, int index, long hurt) {
		try {
			if(!this.dpsIndexMap.containsKey(activeId)){
				return;
			}
			DpsIndex dpsIndex = this.getDpsIndex(activeId, index, role.getLevel());
			if(null == dpsIndex){
				return;
			}
	...
	private void sendRewardMail(RoleInstance role, DpsReward reward, int index, long hurt){
		try {
			...
			GameContext.getMailApp().sendMailAsync(role.getRoleId(), reward.getTitle(), content, 
					...
					0, reward.getBindMoney(), reward.getSilverMoney(), reward.getExp(), reward.getRewardGoodsList());
		} catch (Exception e) {
