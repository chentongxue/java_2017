...
public abstract class BaseEquipBackpack extends DefaultBackpack{
	...
	public int totalEffectStrengthenLevel(int level){
		...
		for(int i=0;i<this.grids.length;i++){
			...
			RoleGoods rg = this.grids[i];
			if(null == rg || rg.getStarNum() < level){
				continue ;
			}
			total ++ ;
		}
	...
	private int getEquipLocation(RoleGoods roleGoods){
		...
		/*else if(goodsBase instanceof GoodsFashion){
			GoodsFashion fashion = (GoodsFashion)goodsBase;
			return fashion.getConditionType();
		}*/
		return -1;
	}
	...
	protected List<RoleGoods> selectFromStorage(){
		 return GameContext.getBaseDAO().selectList(RoleGoods.class, "roleId", this.currentRoleId(), 
					"storageType",this.getStorageType().getType());
	}
	...
	protected void initGoods(List<RoleGoods> list){
		if(null == list){
			return ;
		}
	...
	public Map<Short,Byte> getSuitInfo(){
		Map<Short,Byte> suitMap = new HashMap<Short,Byte>();
		for(int i=0;i<this.grids.length;i++){
			RoleGoods rg = this.grids[i];
			if(null == rg){
				continue ;
			...
			if(null == gb || gb.getSuitId() <=0){
				continue ;
			}
			Byte value = suitMap.get(gb.getSuitId());
			if(null == value){
				suitMap.put(gb.getSuitId(), (byte)1);
			}else{
				suitMap.put(gb.getSuitId(), (byte)(value + 1));
			}
		}
		return suitMap ;
	}
