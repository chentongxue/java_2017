...
public class UseGoodsHorse extends AbstractGoodsBehavior{
	...
	public GoodsResult operate(AbstractParam param) {
		...
		try {
			useResult = GameContext.getHorseApp().useHorseGoods(role, roleGoods);
		} catch (Exception e) {
		...
		//push通知客户端
		String tips = GameContext.getI18n().messageFormat(TextId.HORSE_GOODS_USE_SUCCESS_TIPS,goodsHorse.getName());
