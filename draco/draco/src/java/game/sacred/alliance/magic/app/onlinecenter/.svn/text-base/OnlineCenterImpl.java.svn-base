...
//		}
		...
		// VIP new
		try {
			GameContext.getVipApp().offline(role);
		} catch (Exception e) {
			Log4jManager.OFFLINE_ERROR_LOG.error(
					"VipApp.offline error, roleId=" + roleId + ",userId="
					+ userId, e);
		}
		...
		//英雄试练（必须放到HeroApp之前）
		GameContext.getHeroArenaApp().logout(role);
		//英雄
		...
		//炸金花任务
		GameContext.getQuestPokerApp().logout(role);
		//秘药
