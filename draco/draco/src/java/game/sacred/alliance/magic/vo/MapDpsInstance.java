...
public class MapDpsInstance extends MapLineInstance{
	...
	private void activeOverBroadcast(){
		try {
			...
			ChannelType channlType = ChannelType.getChannelType(mapCfg.getBroadcastChannel());
			if (null == channlType) {
				return;
			}
			txt = txt.replace(Wildcard.MapName, map.getMapConfig()
					.getMapdisplayname().replace(Wildcard.Map_LineId,
							this.lineId + ""));
			GameContext.getChatApp().sendSysMessage(ChatSysName.System, channlType, txt, null, this);
		}catch(Exception ex){
			logger.error("",ex);
		}
	}
	...
	protected void kickRole(RoleInstance role){
		try{
			Point point = this.active.getEnterPoint(role.getCampId());
			if(null == point){
				return;
			}
		...
		try {
			...
			for(int i=0; i<size; i++){
				try {
					...
					GameContext.getActiveDpsApp().sendReward(this.activeId, role, i+1, hurt);
					if(this.buffId > 0){
						//发完奖加buff
