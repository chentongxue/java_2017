...
public class JumpMapPoint extends EventPoint {
	...
	public String isSatisfyCond(AbstractRole role){
		 ...
		 */
		if (questid > 0) {
			if (!((RoleInstance) role).hasFinishQuest(this.questid)) {
				return FALSE;
			}
		}
		if (goodsid > 0) {
			...
			if (!GameContext.getUserGoodsApp().isExistGoodsForBag((RoleInstance)role, goodsid)); {
				return FALSE;
			}
		}
...
//		if(toMap.getMapConfig().getMaplogicid() 
//				== MapLogicType.factionLogic.getType()){
//			//公会地图必须要有公会
//			if(((RoleInstance)role).hasUnion()){
//				return TRUE ;
//			}
//			//this.notifyPointMessage("没有公会无法进入公会领地", (RoleInstance)role);
//			return FALSE;
//		}
	...
	/*private void notifyPointMessage(String message,RoleInstance role){
		// 通知用户跳转失败
		Date d = new Date();
		if (d.getTime() - ((RoleInstance) role).getCopyTime() < LoopConstant.MAP_COPY_TIME) {
			return ;
		}
		ErrorRespMessage pointMsg = new ErrorRespMessage();
		pointMsg.setInfo(message);
		role.getBehavior().sendMessage(pointMsg);
		((RoleInstance) role).setCopyTime(d.getTime());
	}*/
