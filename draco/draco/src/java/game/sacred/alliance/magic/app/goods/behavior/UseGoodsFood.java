...
public class UseGoodsFood extends AbstractGoodsBehavior{
	...
	private String canUse(RoleInstance role, GoodsFood food){
		...
		if (diffTime >= item.getIntervalTime()) {
			...
			return GameContext.getI18n().messageFormat(TextId.USE_GOODS_FOODS_CD, (int)((item.getIntervalTime() - diffTime)/1000));
		}
	...
	private Result useEffered(RoleInstance role, GoodsFood food){
		...
		if(triggerBuffId > 0 && triggerBuffLv >0){
			empty = false ;
			//添加buff
			GoodsBuff goodsBuff = (GoodsBuff) GameContext.getBuffApplication().getBuff(triggerBuffId);
			BuffDetail buffDetail = goodsBuff.getBuffDetail(triggerBuffLv);
			result = GameContext.getUserBuffApp().addBuffStat(role, 
					role,triggerBuffId,buffDetail.getPersistTime(), triggerBuffLv);
			if(!result.isSuccess()){
				return result ;
			}
		}
