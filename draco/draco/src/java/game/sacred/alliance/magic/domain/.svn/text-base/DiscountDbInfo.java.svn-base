...
public @Data class DiscountDbInfo {
	...
	public int getCondCount(int index){
		switch(index){
		case 0:
	...
	public int calcCurDayTotal(Discount discount, Date now, int value){
		int diffDay = DateUtil.dateDiffDay(discount.getStatStartDate(), now);
		if(this.extraInfo == null || this.extraInfo.equals("")){
			this.extraInfo = String.valueOf(diffDay);
			return value;
		}
		//相对也统计开始时间的天数
		int dayFromStatDate = Integer.valueOf(this.extraInfo);
		//同一天
		if(diffDay == dayFromStatDate){
			return this.curDayTotal + value;
		}
		//不是同一天
		this.extraInfo = String.valueOf(diffDay);
		return value;
	...
	public int getRewardCountSum(){
		return this.reward1Count + this.reward2Count + this.reward3Count
			+ this.reward4Count + this.reward5Count + this.reward6Count
			+ this.reward7Count + this.reward8Count + this.reward9Count
			+ this.reward10Count;
	}
