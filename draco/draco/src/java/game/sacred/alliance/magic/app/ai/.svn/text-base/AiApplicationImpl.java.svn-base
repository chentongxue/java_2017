...
public class AiApplicationImpl extends AiApplication {
	...
	public Ai getAi(AbstractRole role) {
		if(RoleType.NPC == role.getRoleType()){
			...
			Ai ai = aiMap.get(self.getNpc().getNpcid());
			Ai selfAi = null ;
			if(null == ai){
				//或者给默认AI
	...
	public void start() {
		try {
			...
			//注册普通NPC AI
			for(Iterator<NormalAiConfig> it = npcAiMap.values().iterator();it.hasNext();){
			     ...
			     aiCfg.init();
			     ConfigurableAi ai = new ConfigurableAi();
			     ai.setAiId(aiCfg.getId());
			     AiApplication.registerAi(ai);
			}
		} catch (Exception e) {
