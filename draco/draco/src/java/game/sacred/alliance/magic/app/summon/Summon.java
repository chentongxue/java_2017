...
public @Data class Summon implements KeySupport<Integer>{
	...
	public Status isMeet(RoleInstance role){
		if(!isInDate()){
			return Status.Summon_Not_InDate;
		}
		if(!isInDay()){
			return Status.Summon_Not_InDate;
		}
		if(!isHasTimes(role)){
			return Status.Summon_Frequency_Not_Enough;
		}
		if(!isMeetConditions(role)){
			return Status.Summon_Condition_Not_Meet;
		}
		if(!isHasConsumeGoods(role)){
			return Status.Summon_ConsumeGood_Not_Enough;
		}
		if(!isHasEnoughMoney(role)){
			return Status.Summon_Menoy_Not_Enough;
		}
		if(!isHasEnoughContribute(role)){
			return Status.Summon_Faction_Contribute_Not_Enough;
		}
		/*
		...
		*/
		return Status.Summon_Can_Summon;
	}
	...
	public boolean isHasTimes(RoleInstance role){
		...
		if(frequencyType == FrequencyType.FREQUENCY_TYPE_MONTH.getType()){
			...
			if(isSameMonth && summonDbInfo.getTimes() >= frequencyValue){
				return false;
			}
			if(!isSameMonth){
				summonDbInfo.setTimes((byte)0);
		...
		if(frequencyType == FrequencyType.FREQUENCY_TYPE_COPY.getType() && summonDbInfo.getTimes() >= frequencyValue){
			return false;
		}
		return true;
	}
	...
	public void updateDbInfo(RoleInstance role){
		...
		if(frequencyType == FrequencyType.FREQUENCY_TYPE_DAY.getType()){
			if(DateUtil.sameDay(now, lastExTime)){
				summonDbInfo.setLastExTime(now);
				summonDbInfo.setTimes((byte)(summonDbInfo.getTimes() + 1));
			}
			else{
				summonDbInfo.setLastExTime(now);
				summonDbInfo.setTimes((byte)1);
			}
			return ;
		}
	...
	public boolean isHasEnoughMoney(RoleInstance role){
		//money
		if(role.getSilverMoney() < consumeSilver 
				|| role.getBindingGoldMoney() < consumeBindingGold
...
//	}
	 ...
	 * @param summonDbInfo
	 */
	public void resetSummon(SummonDbInfo summonDbInfo) {
