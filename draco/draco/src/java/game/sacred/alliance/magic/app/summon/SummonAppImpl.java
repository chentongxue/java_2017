...
public class SummonAppImpl implements SummonApp {
	...
	public Status canSummon(RoleInstance role, Summon summon) {
		try{
			return summon.isMeet(role);
		}catch(Exception e) {
			this.logger.error("",e);
		}
		return Status.Summon_Can_Not_Summon;
	}
	...
	public Result summon(RoleInstance role, Summon summon) {
		...
		int consumBindingGold = summon.getConsumeBindingGold();
		if (consumBindingGold > 0) {
			GameContext.getUserAttributeApp()
					.changeRoleMoney(role, AttributeType.bindingGoldMoney,
							OperatorType.Decrease, consumBindingGold,
							OutputConsumeType.goods_summon_consume);
			needNotify = true;
		}
		
//		//消耗贡献度
//		int consumeContribute = summon.getConsumeContribute();
//		if (consumeContribute > 0) {
//			GameContext.getFactionApp().changeContributeNum(role, OperatorType.Decrease, consumeContribute);
//			needNotify = true;
//		}
//		
//		//消耗贡献度
//		int factionMoney = summon.getConsumeFactionMoney();
//		if (factionMoney > 0) {
//			GameContext.getFactionFuncApp().changeFactionMoney(role, OperatorType.Decrease, factionMoney, OutputConsumeType.faction_money_summon);
//		}
			...
			for(AbstractRole itemRole : ownerInstance.getMapInstance().getRoleList()){
				...
				if (gainSilver > 0) {
					GameContext.getUserAttributeApp().changeRoleMoney(itemRole,
							AttributeType.silverMoney, OperatorType.Add, gainSilver,
							OutputConsumeType.goods_summon_output);
					needNotify = true;
				}
				int gainBindingGold = summon.getAllGainBindingGold();
				if (gainBindingGold > 0) {
					GameContext.getUserAttributeApp().changeRoleMoney(itemRole,
							AttributeType.bindingGoldMoney, OperatorType.Add,
							gainBindingGold, OutputConsumeType.goods_summon_output);
					needNotify = true;
				}
				...
				int contribute = summon.getAllContribute();
				if (contribute > 0) {
//					GameContext.getFactionFuncApp().changeContributeNum((RoleInstance)itemRole, OperatorType.Add, contribute);
					needNotify = true;
				}
				
				/*int magicSoul = summon.getAllMagicSoul();
				if (magicSoul > 0) {
					GameContext.getUserAttributeApp().changeAttribute(itemRole,
							AttributeType.magicSoul, OperatorType.Add, magicSoul,
							OutputConsumeType.goods_summon_output);
					needNotify = true;
				}*/
				if (needNotify) {
