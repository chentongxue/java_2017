...
public class UserGoodsAppImpl implements UserGoodsApp {
	...
	public RoleGoods getRoleGoods(RoleInstance role, StorageType storageType,
		...
		DefaultBackpack storage = this.getStorage(role, storageType);
		if (null == storage) {
			return null;
		}
	...
	/** 返回容器对象 */
	@Override
	public DefaultBackpack getStorage(RoleInstance role, StorageType storageType) {
		...
		case equip:
			return role.getEquipBackpack();
		case hero:
			return GameContext.getUserHeroApp().getEquipBackpack(
					role.getRoleId());
		case goddess:
			return GameContext.getUserGoddessApp().getGoddessEquipBackpack(
					role.getRoleId());
		}
		return null;
	}
	...
	public void deleteRoleGoods(RoleInstance role, RoleGoods roleGoods,
		...
		if (StorageType.equip.getType() == storageType) {
			role.getEquipBackpack().remove(roleGoods);
			return;
		}
	}
	 ...
	 */
	@Override
	public void loadRoleAllGoods(RoleInstance role) {
		initRoleBackpack(role);
		initEquipBackpack(role);
	}
	...
	public void offlineLog(RoleInstance role) {
		try {
			...
			rgList.addAll(role.getEquipBackpack().getAllGoods());
			if (rgList.size() == 0) {
				return;
			}
	...
	public AttriBuffer getAttriBuffer(RoleInstance role) {
		...
		for (RoleGoods roleGoods : role.getEquipBackpack().getAllGoods()) {
	...
	public void createRoleInitGoods(RoleInstance role)
		...
		int equipCount = ParasConstant.ROLE_EQUIP_MAX_NUM;
		RoleBackpack backpack = new RoleBackpack(role, gridCount);
		EquipBackpack equipPack = new EquipBackpack(role, equipCount);
		role.setRoleBackpack(backpack);
		role.setEquipBackpack(equipPack);
		...
		for (int[] novice : noviceList) {
			...
			if (novice[1] == StorageType.equip.getType()) {
				gridIndex = ((GoodsEquipment) goodsBase).getEquipslotType();
				bindType = equipPack.getBindingType(bindingType).getType();
			} else {
	...
	private int getResId(RoleInstance role, EquipslotType locationType) {
		EquipBackpack equipBackpack = role.getEquipBackpack();
		RoleGoods rg = equipBackpack.getEquipGoods(locationType.getType());
	...
	@Override
	public int getResIdByGoods(RoleInstance role, EquipslotType locationType) {
		if (null == locationType || null == role) {
			return 0;
		}
		EquipslotType matchType = null;// TODO:ConditionType.get(locationType.getResMeetPos());
		if (null == matchType) {
			return this.getResId(role, locationType);
		}
		EquipslotType masterType = locationType;
		EquipslotType slaveType = matchType;
		if (matchType.getType() > locationType.getType()) {
			masterType = matchType;
			slaveType = locationType;
		}
		// 系统设置中是否显示时装
		if (role.getSystemSet().isFactionShow()) {
			int resId = this.getResId(role, masterType);
			if (resId > 0) {
				return resId;
			}
		}
		return this.getResId(role, slaveType);
	...
	/* 加载角色装备背包 */
	private void initEquipBackpack(RoleInstance role) {
		int equipCount = ParasConstant.ROLE_EQUIP_MAX_NUM;
		EquipBackpack equipPack = role.getEquipBackpack();
		if (equipPack == null) {
			equipPack = new EquipBackpack(role, equipCount);
		}
		role.setEquipBackpack(equipPack);
	}
