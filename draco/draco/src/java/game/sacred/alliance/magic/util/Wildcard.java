...
public class Wildcard {
	...
	public static String getChatGoodsContent(GoodsBase gb, ChannelType channelType){
		return getChatGoodsContent(gb,channelType,1);
	}
	...
	public static String getChatGoodsContent(GoodsBase gb, ChannelType channelType,int goodsNum){
		...
		buffer.append("[\\P]") .append(chatGoodsCommandId).append(":").append(gb.getId())
			  .append("[\\P\\]").append(getQualityGoodsName(gb)).append("[P]");
			
		//String chatGoodsName = getQualityGoodsName(gb);
		//String info = "[\\P]" + chatGoodsCommandId + ":" + gb.getId() + "[\\P\\]" + chatGoodsName + "[P]";
		if(goodsNum > 1){
	...
	public static String getWingGridName(String wingGridName, QualityType qualityType,ChannelType channelType){
		return Util.getColor(qualityType.getColor()) + "[" + wingGridName + "]" + Util.getColor(channelType.getColor());
	}
}
