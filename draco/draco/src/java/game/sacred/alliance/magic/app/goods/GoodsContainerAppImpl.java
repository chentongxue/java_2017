...
public class GoodsContainerAppImpl implements GoodsContainerApp{
	...
	private void expand(RoleInstance role,GoodsContain contain,boolean confirm){
		...
		if(!confirm){
			...
			ShopGoods shop = GameContext.getShopApp().getShopGoods(realContain.getId());
			int price = shop.getDisGoldPrice() ;
		...
		//扣钱
		ShopGoods shop = GameContext.getShopApp().getShopGoods(contain.getId());
		if(null == shop || shop.getDisGoldPrice() <= 0 ){
		...
		GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.goldMoney, OperatorType.Decrease,
				shop.getDisGoldPrice(), OutputConsumeType.expansion_pack) ;
	...
	private GoodsContain checkGoodsId(int goodsId){
		...
		ShopGoods shop = GameContext.getShopApp().getShopGoods(goodsId);
		if(null == goods || null == shop || shop.getDisGoldPrice() <=0 ){
			Log4jManager.CHECK.error("ContainerConfig config error,goods not exist " +
