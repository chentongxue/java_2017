...
public class GoodsSynchDataAction extends BaseAction<C0503_GoodsSynchDataReqMessage> {
	private List<StorageType> storageTypeList = Lists.newArrayList(
			StorageType.equip,StorageType.bag,StorageType.goddess,StorageType.hero) ;
	...
	public Message execute(ActionContext context, C0503_GoodsSynchDataReqMessage reqMessage) {
		try {
			...
			for(StorageType st : storageTypeList){
				DefaultBackpack pack = GameContext.getUserGoodsApp().getStorage(role, st);
				if(null == pack){
					continue ;
				}
