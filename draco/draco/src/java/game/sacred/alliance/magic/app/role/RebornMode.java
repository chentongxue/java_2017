...
public @Data class RebornMode {
	...
	public void init(String xlsInfo){
		...
		//原地复活，必须配复活道具或者元宝数
		if(RebornType.place.getId() == this.id){
			...
			ShopGoods shopGoods = GameContext.getShopApp().getShopGoods(this.goodsId);
			if(null != shopGoods && shopGoods.getDisGoldPrice() != this.goldMoney){
