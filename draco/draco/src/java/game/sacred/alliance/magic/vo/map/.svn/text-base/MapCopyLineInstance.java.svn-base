...
public class MapCopyLineInstance extends MapInstance {
	 ...
	 * @param role
	 */
	private void reduceCopyConsume(RoleInstance role){
		try {
			int power = this.copyLineConfig.getPower();
			if(power <= 0){
				return;
			}
			//扣除体力值
			...
			logger.error(this.getClass().getName() + ".reduceCopyConsume error: ", e);
		}
	}
