...
public class ConfigurableAi extends DefaultAi {
	...
	public void init() {
		...
		// 1. 添加相关技能
		// 2. 根据npcType设置默认的状态
		aiConfig = context.getAiApplication().getNormalAiConfigMap().get(aiId);
		if (null == aiConfig) {
		...
		// 设置默认状态
		State defaultState = context.getAiApplication().getNpcDefaultState(
				npcRole);
		role.getAi().getStateMachine().switchState(defaultState);
		// 设置攻击距离
		npcRole.setThinkArea(aiConfig.getThinkArea());
		//是否自动调整maxhp
		autoMaxHp = context.getAiApplication().isAutoMapHpNpc(npcRole.getNpc().getNpcid());
	}
	...
	public void enterEvadeMode() {
		...
		State defaultState = GameContext.getAiApplication().getNpcDefaultState(
				(NpcInstance) role);
		role.getAi().getStateMachine().switchState(defaultState);
	...
	private void refreshNpc() {
		try {
			GameContext.getAiApplication().deathRefresh(
