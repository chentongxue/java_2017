...
public class GoodsDeriveRecastingInfoAction extends BaseAction<C0554_GoodsRecastingInfoReqMessage>{
	...
	public Message execute(ActionContext context, C0554_GoodsRecastingInfoReqMessage reqMsg) {
		...
		try{
			...
			EquipRecatingConfig config = GameContext.getGoodsApp().getEquipRecatingConfig(gb.getQualityType());
			if(null == config){
				//没有洗练配置也让用户看到相关属性
	...
	private List<GoodsRecastingInfoItem> buildItem(RoleGoods equipGoods){
		...
		for(AttriItem item : list){
			...
			EquipRecatingAttrWeightConfig awc = GameContext.getGoodsApp().getEquipRecatingAttrWeightConfig(attriType, gb.getQualityType());
			if(null == awc){
				continue;
			}
