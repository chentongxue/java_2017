...
public @Data class LootList {
	 ...
	 * @return
	 */
	public Map<Integer,Integer> getOnlyGoods(){
		//TODO: 测试
		Map<Integer,Integer> onlyLoot = Maps.newConcurrentMap();
		
		Map<Integer,Integer> values = this.getGoods();
		
		int randomNum = values.size();
		if(randomNum > 1){
			//获取随机数
			randomNum = RandomUtil.randomInt(1,randomNum);
		}
		//按照随机数获得一个掉落物品
		int num = 0;
		for(Iterator<Integer> it = values.keySet().iterator(); it.hasNext();){
			Integer goodsId = it.next();
			Integer goodsNum = values.get(goodsId);
			if(num == (randomNum - 1)){
				onlyLoot.put(goodsId, goodsNum);
				break;
			}
			num++;
		}
