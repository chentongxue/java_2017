...
import com.game.log.actor.game.SamsaraBox;
import com.game.log.actor.game.SamsaraPay;
import com.game.log.actor.game.Task;
import com.game.log.actor.game.Tower;
...
public class StatLogAppImpl implements StatLogApp{
	...
	public void mailSendInfoPickLog(RoleInstance role,Mail mail,List<MailAccessory> maList,
			int exp,int gold,int bindGold,int silverMoney,int contribute,int zp,int magicSoul){
		try{
			StringBuffer logContent = new StringBuffer();
			this.packGoods(role, maList, logContent);
			this.packMoneyExp(exp, gold, bindGold, silverMoney, contribute, zp, magicSoul, logContent);
	...
	//封装邮件的钱，经验日志
	private void packMoneyExp(int exp,int gold,int bindGold,int silverMoney,int contribute,int zp,int magicSoul,StringBuffer logContent){
		try{
			if(gold > 0){
			...
			if(bindGold > 0){
				logContent.append(AttributeType.bindingGoldMoney.getName());
				logContent.append("*");
				logContent.append(bindGold);
				logContent.append(";");
			}
			if(silverMoney > 0){
				logContent.append(AttributeType.silverMoney.getName());
				logContent.append("*");
		...
		try {
			...
			for (RoleGoods rg : rgList) {
				try {
					...
					info.setStarNum(String.valueOf(rg.getStarNum()));
...
//	}
	...
	@Override
	public void samsaraBoxLog(RoleInstance role ,int chestType){
		try{
			SamsaraBox info = new SamsaraBox();
			info.setBoxType(String.valueOf(chestType));
			info.setRoleId(role.getRoleId());
			info.setRoleLevel(String.valueOf(role.getLevel()));
			info.setRoleName(role.getRoleName());
			this.execute(role, info);
	...
	@Override
	public void samsaraPayLog(RoleInstance role,int paidTimes){
		try{
			SamsaraPay info = new SamsaraPay();
			info.setPaidTimes(String.valueOf(paidTimes));
			info.setRoleLevel(String.valueOf(role.getLevel()));
			info.setRoleName(role.getRoleName());
			info.setRoleId(role.getRoleId());
			this.execute(role, info);
	...
	@Override
	public void towerLog(RoleInstance role,MapConfig mc ,String scoreLeve,int exp,int isSuccess){
		try{
			//int refreshNum = GameContext.getTowerApp().getRefreshNum(role);
			Tower info = new Tower();
			info.setChallengeResult(String.valueOf(isSuccess));
			info.setRoleId(role.getRoleId());
			info.setRoleLevel(String.valueOf(role.getLevel()));
			info.setRoleName(role.getRoleName());
			info.setTowerCrossing(String.valueOf(mc.getCurrGateMapIndex()));
			info.setTowerExperience(String.valueOf(exp));
			info.setTowerScore(scoreLeve);
			info.setTowerType(String.valueOf(mc.getGateIndex()));
			//info.setResetNum(String.valueOf(refreshNum));
			this.execute(role, info);
		}catch(Exception e){
			logger.error("",e);
		}
