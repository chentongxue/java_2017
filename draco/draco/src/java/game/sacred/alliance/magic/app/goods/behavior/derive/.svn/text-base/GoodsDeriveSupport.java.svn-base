...
public abstract class GoodsDeriveSupport {
	...
	public static RoleGoods getRoleGoods(RoleInstance role, byte bagType, String goodsInstanceId) {
	...
	public static void payMoney(RoleInstance role, int gameMoney, OutputConsumeType ocType) {
		...
		GameContext.getUserAttributeApp().changeRoleMoney(role,
				AttributeType.silverMoney, OperatorType.Decrease, gameMoney,ocType);
	...
	public static GoodsGem getGemTemplate(int id){
		if(id <=0){
			return null ;
		}
		GoodsBase gb = null ;
