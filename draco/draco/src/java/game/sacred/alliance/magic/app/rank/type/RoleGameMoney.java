...
/**
 * /**
 * log日志格式：roleId#gameMoney#roleId#roleName#gender#career#level#campId#factionId
 * 返回的日志格式：sort#log日志格式
 * 排行榜小类型和careerType对应，根据排行榜小类型判断是否打印日志
 */
public class RoleGameMoney extends RankRoleLogic {

	private static RoleGameMoney instance = new RoleGameMoney();
	private RoleGameMoney(){
	}
	
	public static RoleGameMoney getInstance(){
		return instance ;
	}
	...
	public void initLogData(RankInfo rankInfo){
		List<RoleInstance> roleList = GameContext.getRankDAO().selectGameMoneyRole("career", 
				rankInfo.getSubType(), 
	...
	public RankType getRankType() {
		return RankType.Role_Game_Money ;
	}
