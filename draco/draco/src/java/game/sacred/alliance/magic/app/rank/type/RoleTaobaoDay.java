...
/**
 * 
 * log日志格式：roleId#taobaotimes#roleId#roleName#gender#career#level#campId#factionId
 * 返回的日志格式：sort#log日志格式
 * 过滤条件:淘宝类型
 */
public class RoleTaobaoDay extends RankRoleLogic{

	private static RoleTaobaoDay instance = new RoleTaobaoDay();
	private RoleTaobaoDay(){
	}
	
	public static RoleTaobaoDay getInstance(){
		return instance ;
	}
	...
	private int getData(RoleInstance role, RankInfo rankInfo){
		...
		return count.getTaobao(subType);
	}
	...
	public RankType getRankType() {
		return RankType.Role_Taobao_Day ;
	}
	...
	public void initLogData(RankInfo rankInfo){
		List<RoleCount> roleCountList = GameContext.getRankDAO().selectTaobao("compassType", 
