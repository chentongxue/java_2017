...
/**
 * 
 * log日志格式：roleId#recharge#roleId#roleName#gender#career#level#campId#factionId
 * 返回的日志格式：sort#log日志格式
 * 排行榜小类型和careerType对应，根据排行榜小类型判断是否打印日志
 */
public class RoleRecharge extends RankRoleLogic {

	private static RoleRecharge instance = new RoleRecharge();
	private RoleRecharge(){
	}
	
	public static RoleRecharge getInstance(){
		return instance ;
	}
	...
	public void initLogData(RankInfo rankInfo){
		List<RoleInstance> roleList = GameContext.getRankDAO().selectRechargeRole("career", 
				rankInfo.getSubType(), 
