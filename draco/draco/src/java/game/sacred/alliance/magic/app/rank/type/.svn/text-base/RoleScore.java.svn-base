 ...
 */
public class RoleScore extends RankRoleLogic {
	private static RoleScore instance = new RoleScore();
	private RoleScore(){
	}
	
	public static RoleScore getInstance(){
		return instance ;
	}
	...
	protected void doPrintLog(RoleInstance role, RankInfo rankInfo) {
		...
		sb.append(CAT);
		sb.append(this.getRankBattleScore(role));
		sb.append(CAT);
	...
	public void initLogData(RankInfo rankInfo){
		List<RoleInstance> roleList = GameContext.getRankDAO().selectScoreRole("career", 
				rankInfo.getSubType(), 
