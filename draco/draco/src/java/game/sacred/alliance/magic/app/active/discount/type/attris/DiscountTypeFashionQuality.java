...
public class DiscountTypeFashionQuality extends DiscountTypeUpdate {
	...
	public boolean updateCondCount(RoleInstance role, DiscountDbInfo discountDbInfo, Discount discount, int value) {
		...
		//取人身上时装信息
		List<RoleGoods> fashions = role.getEquipBackpack().getFashionGoods();
		if(Util.isEmpty(fashions)){
			return false;
		}
		//<装备品质，装备数量>
		Map<Byte, Byte> qualityNumMap = new HashMap<Byte, Byte>();
		for(RoleGoods goods : fashions){
			if(null == goods){
				continue;
			}
