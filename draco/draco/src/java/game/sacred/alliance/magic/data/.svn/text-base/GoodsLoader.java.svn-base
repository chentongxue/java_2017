...
import sacred.alliance.magic.app.title.TitleInitCallback;
...
public class GoodsLoader extends DefaultDataLoader<String, GoodsBase> {
	...
	private List<LoadMapping> getMappings(){
		...
		//宝石
		mappings.add(new LoadMapping("goods_gem.xls","goods_gem",GoodsGem.class,null,GoodsType.GoodsGem));
		//时装 时装合并为装备类型
		//mappings.add(new LoadMapping("goods_fashion.xls","goods_fashion",GoodsFashion.class, null,GoodsType.GoodsFashion));
		//家园材料
		...
		//升级礼包
		mappings.add(new LoadMapping("goods_gift.xls","goods_gift",GoodsGift.class,null,GoodsType.GoodsGift));
		//公会
//		mappings.add(new LoadMapping("goods_faction.xls","faction",GoodsFaction.class,null,GoodsType.GoodsFaction));
		...
		//女神
		mappings.add(new LoadMapping("goods_goddess.xls","list",GoodsGoddess.class,null,GoodsType.GoodsGoddess));
		//秘药物品
