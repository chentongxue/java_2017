...
public class MapUnionInstance extends MapInstance implements java.io.Serializable{
	...
	public void initNpc(boolean loadNpc) {
		...
		for (NpcBorn npcBorn : listNpcBorn) {
			boolean flag = isCreateNpc(npcBorn.getBornnpcid());
			if(!flag){
				continue;
			}
			// 获得NPC模板
	...
	protected void npcDeathDiversity(AbstractRole attacker, AbstractRole victim){
		...
		GameContext.getUnionInstanceApp().saveOrUpdUnionKillBossRecord(role.getUnionId(),  npc.getNpcid());
	}
	...
	public void npcDeath(NpcInstance npc) {
		...
		if(isBoss(npc.getNpcid())){
			...
			if(groupId != -1){
				if(roleDpsMap.containsKey(groupId)){
					java.util.Map<Integer,RoleDps> dpsMap = roleDpsMap.get(groupId);
					for(Entry<Integer,RoleDps> dps : dpsMap.entrySet()){
	...
	private boolean isCreateNpc(String npcId){
		if(isBoss(npcId)){
			...
			byte state = GameContext.getUnionInstanceApp().getInsBossState(npcId, activityId, groupId);
			if(state == 0){
				return true;
			}
		}
