...
public class UseGoodsTreasure extends GoodsTreasureBehavior {
	...
	public Result operate(AbstractParam param) {
		...
		if(useResult.isSuccess()){
			...
			//更新用户计数
			GameContext.getCountApp().updateTreasureMap(role, (byte)treasure.getQualityType());
			return useResult;
		}
		return useResult;
	}
	...
	private Result canPositionUse(RoleInstance role, String otherParam, GoodsTreasure treasure){
		...
		if(distance <= treasure.getRadius()){
			//可以成功使用
	...
	private GoodsResult useTreasure(RoleInstance role, GoodsTreasure treasure,
		...
		try{
			...
			case 1: //触发怪物
				...
				for(String key : monster.getMonstersMap().keySet()){
					createTreasureMonster(role, key, monster.getMonstersMap().get(key));
				}
				result.success();
				tips = new C0006_TipNoSelectWindowNotifyMessage();
				tips.setMsgContent(GameContext.getI18n().getText(TextId.TREASURE_GET_MONSTER));
				role.getBehavior().sendMessage(tips);
				//广播
				try{
			...
			case 2: //获得物品
				...
				if (bindMoney > 0) {
					GameContext.getUserAttributeApp().changeRoleMoney(role,
							...
							bindMoney, OutputConsumeType.treasure_map);
					needNotify = true;
				}
				if (gameMoney > 0) {
					GameContext.getUserAttributeApp().changeRoleMoney(role,
							AttributeType.silverMoney, OperatorType.Add,
	...
	private void createTreasureMonster(RoleInstance role, String npcId, int npcNum) {
		try{
			...
			for(int i=0; i<npcNum; i++){
				mapInstance.summonCreateNpc(npcId, roleMapX, roleMapY);
			}
		}catch(Exception e) {
