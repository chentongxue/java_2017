...
public @Data class Active {
	...
	private byte minLevel;//最小角色等级
	private byte maxLevel;//最大角色等级
	...
	public boolean needReset(RoleInstance role){
		ActiveLogInfo samInfo = role.getActiveInfo(id);
		if(null==samInfo || samInfo.getActiveId()!=id){
			return false;
		}
		Date resetTime = samInfo.getResetTime();
		byte days = getResetCycle();
		String time = getResetTime();
		return DateUtil.isResetTime(resetTime, days, time);
	}
	...
	private ActiveSupport getActiveSupport(){
		return GameContext.getActiveApp().getActiveSupport(Integer.valueOf(type));
	}
