...
public class GoodsRecasting extends AbstractGoodsBehavior {
	...
	public Result operate(AbstractParam param) {
		...
		//新属性生效
		this.updateRoleAttr(role, result);
	...
	private void baseCond(RoleInstance role, RecastingParam input, RecastingResult result) {
		...
		EquipRecatingConfig config = GameContext.getGoodsApp().getEquipRecatingConfig(template.getQualityType());
		if (null == config) {
			result.setInfo(GameContext.getI18n().getText(TextId.GOODS_RECASTING_NOT_RECASTING));
			return;
		}
	...
	private ArrayList<AttriItem> randomAttrList(RecastingResult result, RecastingParam input, int operateType, int rmAttrNum){
		...
		int qualityType = result.getTemplate().getQualityType();
		...
		for(byte attrType : attrList){
			EquipRecatingAttrWeightConfig attrWeight = GameContext.getGoodsApp().getEquipRecatingAttrWeightConfig(attrType, qualityType);
			if(null == attrWeight){
				continue;
			}
		...
		if(isUseGold && !hasMaxQuality){
			...
			if(Util.isEmpty(result.getEquipGoods().getAttrVarList())){
				...
				EquipRecatingAttrWeightConfig attrWeight = GameContext.getGoodsApp().getEquipRecatingAttrWeightConfig(firstType, qualityType);
				if(null != attrWeight){
	...
	private void updateRoleAttr(RoleInstance role, RecastingResult result) {
		...
		boolean on = (bagType == StorageType.equip.getType());
		...
		if (on) {
			// 穿着的装备才需要重新计算属性
			// 需要重新计算属性
			boolean onEquipBag = bagType == StorageType.equip.getType();
