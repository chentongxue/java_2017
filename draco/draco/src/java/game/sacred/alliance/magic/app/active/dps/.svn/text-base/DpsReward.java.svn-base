...
public @Data class DpsReward {
	...
	public Result checkAndInit(){
		...
		try {
			...
			if(this.bindMoney > 0){
				this.showAttrList.add(new AwardAttrItem(AttributeType.bindingGoldMoney.getType(), this.bindMoney));
			}
			if(this.silverMoney > 0){
				this.showAttrList.add(new AwardAttrItem(AttributeType.silverMoney.getType(), this.silverMoney));
			}
