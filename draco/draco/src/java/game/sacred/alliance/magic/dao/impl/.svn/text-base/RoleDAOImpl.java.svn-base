...
public class RoleDAOImpl extends SqlMapClientDaoSupport implements RoleDAO{
	...
	public List<RolePayRecord> getUserMoneyRankList(int orderby, int start, int end) {
		...
		map.put("start", start);
		map.put("end", end);
		return this.getSqlMapClientTemplate().queryForList("UserPayRecord.getUserMoneyRank", map);
	}
	...
	public long getGoldMoneyReamin() {
		Object object = this.getSqlMapClientTemplate().queryForObject("UserPayRecord.goldMoneyRemain");
		if(null == object) {
			return 0;
		}
		return (Long)object;
