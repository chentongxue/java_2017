...
import com.game.draco.message.item.RoleAttributeFontItem;
import com.game.draco.message.item.RoleBodyItem;
import com.game.draco.message.item.RoleStateFontItem;
...
public class RoleInstanceBehavior extends AbstractRoleBehavior<RoleInstance> {
	...
	private LoopCount defaultLoopCount = new LoopCount(
			LoopConstant.ROLE_DEFAULT_CYCLE);
	...
	private LoopCount goddessLoopCount = new LoopCount(
			LoopConstant.GODDESS_CYCLE);
	...
	public void update() throws ServiceException {
		...
		if (this.goddessLoopCount.isReachCycle()) {
			GameContext.getGoddessApp().goddessWeakTimeOver(role);
		}
	...
	public void notifyBattleAttrIncome(NpcInstance dieNpc) {
		try {
			...
			//绑金
			if(npcTemplate.getBindMoney()>0){
				GameContext.getUserAttributeApp().changeRoleMoney(role, AttributeType.bindingGoldMoney, 
						OperatorType.Add, npcTemplate.getBindMoney(),OutputConsumeType.monster_fall);
				notify = true ;
			}
			//游戏币
	...
	public void addSelfFont(AttrFontSizeType sizeType,
		...
		this.buildFontInfo(sizeType, colorType, value, role.getIntRoleId(), attacker.getIntRoleId());
	}
	...
	public void notifyAttrFont() {
		Map<Integer, List<RoleAttributeFontItem>> fontMap = null;
		...
		for (int i = 0; i < ParasConstant.ATTRFONT_MAX_SIZE; i++) {
			...
			if (null == fontMap) {
				fontMap = new HashMap<Integer, List<RoleAttributeFontItem>>();
			}
			
			List<RoleAttributeFontItem> items = fontMap.get(roleId);
			if (null == items) {
				items = new ArrayList<RoleAttributeFontItem>();
				fontMap.put(roleId, items);
			}
			
			if(attrFont.getSize() == AttrFontSizeType.State.getType()) {
				//状态通知
				RoleStateFontItem item = new RoleStateFontItem();
				item.setAttackerId(attrFont.getAttackerId());
			...
			//属性通知
			RoleAttributeFontItem item = new RoleAttributeFontItem();
			item.setType(attrFont.getFontType());
			item.setValue(attrFont.getValue());
			items.add(item);
		}
		...
		for (Iterator<Map.Entry<Integer, List<RoleAttributeFontItem>>> it = fontMap.entrySet().iterator();it.hasNext();) {
			Map.Entry<Integer, List<RoleAttributeFontItem>> entry = it.next();
			List<RoleAttributeFontItem> items = entry.getValue();
			...
			if (1 == items.size()) {
				...
				RoleAttributeFontItem item = items.get(0);
