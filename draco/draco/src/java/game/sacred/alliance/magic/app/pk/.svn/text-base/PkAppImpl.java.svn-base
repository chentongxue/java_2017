...
public class PkAppImpl implements PkApp{
	...
	public void internalAttackerLogic(RoleInstance attacker, RoleInstance victim){
		try{
			...
			int killNum = attacker.getRoleCount().getKillCount();
			killNum++;
			attacker.getRoleCount().setKillCount(killNum);
			
			if(!victim.hasBuff(protectBuffId)){
				int todayKillNum = attacker.getRoleCount().getTodayKillCount();
				todayKillNum++;
				attacker.getRoleCount().setTodayKillCount(todayKillNum);
			}
	...
	private void addAttackerBuff(RoleInstance role, short buffId, int buffLevel){
		try{
			...
			int effectTime = buff.getPersistTime(buffLevel);
			if(null != buffStat){
	...
	private void punish(AbstractRole attacker, RoleInstance victim, List<DeathNotifySelfItem> optionList){
		...
		//死者是开了屠杀模式的
		int killNum = victim.getRoleCount().getKillCount();
		PkKillConfig kConfig = getPkKillConfig(killNum);
		if(null == kConfig){
			GameContext.getRoleRebornApp().notifySelfDeath(victim, attacker, optionList);
			return;
		...
		if(moneyRate > 0 && moneyRate < PERCENT){
			...
			if(reduceMoney > 0){
				GameContext.getUserAttributeApp().changeAttribute(victim, AttributeType.silverMoney, OperatorType.Decrease, reduceMoney, OutputConsumeType.pk_punish);
				notify = true;
			}
		}
		//减少经验
	...
	public Result changePkStatus(RoleInstance role, byte pkStatus){
		...
		try{
			...
			if(pkStatus == RolePkStatus.MASSACRE.getType()) {
				int killNum = role.getRoleCount().getKillCount();
				PkKillConfig kConfig = getPkKillConfig(killNum);
				if(null == kConfig){
					return result.success();
				}
...
//				}
	...
	public void roleLogin(RoleInstance role){
		try{
			...
			role.setColor(this.getColorByKillNum(role.getRoleCount().getKillCount()));
